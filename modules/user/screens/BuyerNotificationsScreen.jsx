...
function NotificationList({ notifs, onOpen, onMarkAllRead, onBack }) {
  ...
  const filtered = useMemo(() => {
    ...
    else if (filter === 'Messages')    list = list.filter(n => n.type === 'agent_message' || n.type === 'builder_message');
...
function NotificationDetail({ notif, onBack }) {
  const cfg     = TYPE[notif.type];
  const isDeal  = notif.type === 'deal_closed';
  const isAgent = notif.type === 'agent_message';

  const accentColor = isDeal ? T.gold : (isAgent ? T.blue : T.teal);

  const navigate = (action) => {
    const msgs = {
      chat:     `Opening ${isAgent ? 'chat' : 'contact'} with ${notif.sender.name}…`,
      property: `Opening ${notif.property?.name ?? 'property details'}…`,
      call:     `Calling ${notif.sender.name} at ${notif.sender.phone}…`,
    };
    ...
    <View style={gs.screen}>
      ...
      <ScrollView contentContainerStyle={dv.scroll} showsVerticalScrollIndicator={false}>
        ...
        <View style={[dv.senderCard,{ borderLeftColor:accentColor, borderLeftWidth:4 }]}>
          ...
          <View style={{ flex:1 }}>
            <Text style={dv.sName}>{notif.sender.name}</Text>
            <View style={[dv.roleChip,{ backgroundColor:cfg.bg, borderColor:cfg.border }]}>
              <cfg.Icon color={cfg.color} size={11} strokeWidth={2.5} />
              <Text style={[dv.roleChipTxt,{ color:cfg.color }]}>{notif.sender.role}</Text>
            </View>
            ...
            <View style={dv.propCard}>
              ...
              <View style={dv.propBody}>
                ...
                <View style={{ flexDirection:'row', gap:8, flexWrap:'wrap' }}>
                  ...
                  <View style={[dv.propStat,{
                    backgroundColor: notif.property.status==='Closed' ? T.goldBg : T.g100,
                  }]}>
                    <Text style={[dv.propStatTxt,{ color: notif.property.status==='Closed' ? T.gold : T.g700 }]}>
                      {notif.property.status}
                    </Text>
                  </View>
                </View>
              </View>
        ...
        <View style={{ gap:10, marginTop:24 }}>
          ...
          {notif.nav?.chatLabel && (
            ...
            >
              ...
              <Text style={[dv.ctaPrimaryTxt, isDeal && { color:T.gold }]}>{notif.nav.chatLabel}</Text>
              <ChevronRight color={isDeal ? T.gold : T.g800} size={17} strokeWidth={2.5} />
            </TouchableOpacity>
          )}

          {/* Secondary: View Property / Deal Details */}
          {notif.nav?.propertyLabel && notif.property && (
            <TouchableOpacity
            ...
            >
              ...
              <Text style={dv.ctaSecondaryTxt}>{notif.nav.propertyLabel}</Text>
          ...
          >
            ...
            <Text style={dv.ctaTertiaryTxt}>
              Call {notif.sender.name.split(' ')[0]}
            </Text>
